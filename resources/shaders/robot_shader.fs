...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir, Material material)
{
    ...
    vec3 specular = light.specular * spec * material.texture_specular1.xyz *0.0;//vec3(texture(material.texture_specular1, TexCoords).xxx);
    ambient *=  attenuation;
    diffuse *= attenuation;
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir, Material material)
{
    ...
    return (0.1*ambient + 0.5*diffuse); // + 0.01*specular);
}
